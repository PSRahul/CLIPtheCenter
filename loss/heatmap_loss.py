...
def calculate_heatmap_loss(predicted_heatmap, groundtruth_heatmap):
    ...
    heatmap_loss = heatmap_loss_fn(predicted_heatmap.float()+1e-10, groundtruth_heatmap.float())
...
def calculate_heatmap_loss(predicted_heatmap, groundtruth_heatmap):
    heatmap_loss = torch.nn.functional.mse_loss(input=predicted_heatmap.float()+1e-10, target=groundtruth_heatmap.float(),
                                                reduction='mean')
    return heatmap_loss
